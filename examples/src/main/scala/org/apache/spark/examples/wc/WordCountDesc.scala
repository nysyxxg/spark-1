...
object WordCountDesc {
  def main(args: Array[String]) {
    ...
    val lines = sc.textFile("D:\\eip-deciphering\\eip-ods-loader\\eip-ods-loader-test\\src\\main\\data\\wordCount.txt")   // 读取本地文件
    ...
    val words = lines.flatMap(line=>{
     var  data = line.split(" ")
      data.toList
    }).filter(word => word != " ")  // 拆分单词，并过滤掉空格，当然还可以继续进行过滤，如去掉标点符号
